...
public class MainActivity extends AppCompatActivity implements AlarmAdapter.Listener {
    ...
    private void setupAlarmList() {
        ...
        repository.observeAll().observe(this, alarms -> {
            adapter.submit(alarms); empty.setVisibility(alarms==null||alarms.isEmpty()?View.VISIBLE:View.GONE); updateUpcoming(alarms);
        });
    }
    ...
    private void scheduleQuick(int minutes) {
        if(!ensureExactPermission())return; AlarmScheduler.scheduleQuick(this,minutes); Toast.makeText(this,getString(R.string.quick_scheduled,minutes),Toast.LENGTH_LONG).show();
    }
    private void showSettings() {
        ...
        new AlertDialog.Builder(this).setTitle(R.string.appearance_language).setView(content).setNegativeButton(R.string.cancel,null)
    ...
    @Override public void delete(Alarm alarm) { new AlertDialog.Builder(this).setTitle(R.string.delete_alarm_title).setMessage(getString(R.string.delete_alarm_message,alarm.hour,alarm.minute,alarm.label)).setNegativeButton(R.string.cancel,null).setPositiveButton(R.string.delete,(d,w)->repository.delete(alarm)).show(); }
    ...
    @Override protected void onResume() { super.onResume(); if(repository!=null&&AlarmScheduler.canScheduleExact(this))repository.rescheduleAll(); }
}
